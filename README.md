...
2. 微任务因为其高优先级的特性，能确保队列中的微任务在一次事件循环前被执行完
3. 因为兼容性问题，vue不得不做了微任务向宏任务的降级方案
...
### 19.什么是递归组件？
- 组件是可以在它们自己的模板中调用自身的，如果使用相同的模板结构，但数据是分层输入的，就可以使用组件递归来实现
- 组件在递归过程中，一定要有一个结束的条件，不然无限制递归会造成内存溢出使应用崩溃

### 20.谈一谈你对vue响应式原理的理解？
- vue使用Object.defineProperty来对除数组外的数据设置get/set方法来监听数据变化，通过getter进行依赖收集，而每个setter方法就是一个观察者，在数据变更的时候通知订阅者更新视图
- 